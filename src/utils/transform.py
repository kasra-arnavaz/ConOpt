...
class Transform:
    ...
    def apply(self, point: torch.Tensor):
        point = point.to(self._device)
        return self._apply_scale(self._apply_rotation(self._apply_translate(point)))
...
def get_quaternion(vector: list, angle_in_degrees: float) -> list:
    ...
    angle_in_radians = torch.deg2rad(torch.tensor(angle_in_degrees / 2.))
