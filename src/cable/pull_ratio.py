...
class TimeInvariablePullRatio(PullRatio):
    def __init__(
        self, simulation_properties: SimulationProperties, pull_ratio: torch.Tensor = None, device: str = "cuda"
    ):
        self._pull_ratio = (
            pull_ratio if pull_ratio is not None else torch.tensor(0.0, device=device, requires_grad=True)
        )
...
class TimeVariablePullRatio(PullRatio):
    def __init__(
        self, simulation_properties: SimulationProperties, pull_ratio: List[torch.Tensor] = None, device: str = "cuda"
    ):
        self._time = simulation_properties.key_timepoints
        self._pull_ratio = (
            pull_ratio
            if pull_ratio is not None
            else [torch.tensor(0.0, device=device, requires_grad=True) for _ in range(len(self._time))]
        )
