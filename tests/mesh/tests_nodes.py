...
class TestNodes(unittest.TestCase):
    ...
    def tests_if_type_error_is_raised_when_position_is_set_to_list(self):
        with self.assertRaises(TypeError):
            nodes = Nodes(position=torch.zeros([10, 3]))
            nodes.position = [1, 2, 3]
    ...
    def tests_if_value_error_is_raised_when_positions_last_dimension_is_set_to_2(self):
        with self.assertRaises(ValueError):
            p = torch.arange(12).reshape(4, 3)
            nodes = Nodes(position=p)
            nodes.position = torch.arange(12).reshape(6, 2)
    ...
    def tests_if_type_error_is_raised_when_velocity_is_set_to_tensor_of_a_different_shape_to_position(self):
        with self.assertRaises(ValueError):
            p = torch.arange(12).reshape(4, 3)
            nodes = Nodes(position=p)
            nodes.velocity = torch.arange(24).reshape(8, 3)

    def tests_the_length_of_nodes(self):
        p = torch.arange(21).reshape(7,3)
        nodes = Nodes(position=p)
